<template>
  <div class="container">
    <div class="auth-form">
      ...
      <p>На указанный email придет ссылка для сброса пароля.</p>

      <button class="enter-button" @click="resetPassword">Сбросить</button>
    </div>
  </div>
</template>
    ...
    const resetPassword = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          errorMessage.value = errorData.message || 'Ошибка при сбросе пароля';
          console.error('Ошибка при сбросе пароля:', errorData);
        }
      } catch (error) {
