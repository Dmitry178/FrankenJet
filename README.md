...
* **Backend:**
  * Python 3.12, PostgreSQL 17 (AsyncPG, SQLAlchemy 2 ORM, PGVector), API (async FastAPI, WebSocket, AIOHTTP, OAuth2, gRPC), S3 MiniO (AIOBoto3), RabbitMQ (FastStream), Redis (FastApi-Cache2), Elasticsearch, GigaChat API.
* **Frontend:**
  * Vue.js 3.5, Vite, Vuetify, Axios, Pinia, Vue Router.
* **Микросервисы**:
  * Бот уведомлений: Python 3.12, AIOGram3, RabbitMQ (FastStream).
  * Векторизация текста: Python 3.12, gRPC, FastEmbed.
...
* **Backend:**
  ...
  * Поиск по статьям через Elasticsearch (и через SQL-запросы при недоступности Elasticsearch).
...
* **Микросервисы:**
  * Телеграм-бот уведомлений (выводит события в бэкенде, выводит ошибки в логах, получает события об аутентификации администратора и новых комментариях), работает через RabbitMQ.
  * Сервис векторизации текста (через gRPC).
...
* **Функциональность:**
  * Семантический поиск.
  * Блок рекомендаций статей.
  * Чат-бот по теме авиации и информации по проекту (RAG).
