...
Fullstack: Backend (FastAPI) + Frontend (Vue.js 3.5) + Микросервисы.

* **Backend:**
  * Python 3.13, PostgreSQL 17 (AsyncPG, SQLAlchemy 2 ORM, PGVector), API (async FastAPI, WebSocket, AIOHTTP, OAuth2, gRPC), S3 MiniO (AIOBoto3), RabbitMQ (FastStream), Redis (FastApi-Cache2), Elasticsearch, GigaChat API.
* **Frontend:**
  * Vue.js 3.5, Vite, Vuetify, Axios, Pinia, Vue Router.
* **Микросервисы**:
  * Бот уведомлений: Python 3.12, AIOGram3, RabbitMQ (FastStream).
  * Векторизация текста: Python 3.12, gRPC, FastEmbed/Sentence Transformers.
...
│  │  ├─ 📁 services           # HTTP-клиенты, API-запросы
...
│
├─ 🗂️ vectorizer               # Сервис векторизации текста
│  ├─ 📁 app                   # Основной исходный код сервиса
│  ├─ 📁 proto                 # Определение gRPC-сервисов и сообщений
│  ├─ 📁 models                # Директория для кэша (загружаемых моделей)
│  ├─ 📄 Dockerfile            # Определение контейнера сервиса
│  ├─ 📄 docker-compose.yml    # Деплой сервиса векторизации
│  ├─ 📄 models.py             # Вывод списка поддерживаемых моделей
│  ├─ 📄 README.md             # Документация сервиса векторизации
│  └─ 📄 server.py             # Точка входа в приложение векторизации
│
...
* **Backend:**
  * Использованы архитектурные паттерны для изоляции слоёв и масштабируемости приложения.
  ...
  * Контекстные менеджеры для базы данных, HTTP, RabbitMQ, S3, Elasticsearch и связи с микросервисами по gRPC, менеджеры для Redis и WebSocket.
  * Кэширование настроек приложения и статьей.
  * Отправка логов в телеграм-бот уведомлений (уровень от ERROR и выше).
  * Организована логика RAG/AI чат-бота по теме авиации.
  * Скрипты для:
  ...
  * Поиск по статьям через Elasticsearch (и через SQL-запросы при недоступности/ошибке Elasticsearch).
...
* **Frontend:**
  ...
  * Базовая структура приложения:
    * главная страница с 3 случайными статьями (в desktop-версии) и 5 интересными фактами об авиации;
    * страница статей с техническими характеристиками и содержанием;
    * страница с отображением списка статей по тегам;
    * страница результатов поиска по статьям;
    * страница профиля пользователя с загрузкой аватара;
    * страница настроек чат-бота (для администратора).
  * Чат-бот по теме авиации.
  * Поддержка тем (светлая и два варианта тёмной темы).
  * Адаптивный дизайн (десктоп, планшет, телефон).
  * SEO-метаданные статей.

* **Микросервисы:**
  * Телеграм-бот уведомлений (выводит события в бэкенде, выводит ошибки в логах, получает события об аутентификации администратора), работает через RabbitMQ.
  * Сервис векторизации текста через FastEmbed/Sentence Transformers (работает через gRPC).
...
* **Безопасность:**
  ...
  * Оповещение через телеграм-бот о входе пользователя с ролью администратора с возможностью отозвать refresh-токены / заблокировать учётную запись.
